...
func run() error {
	...
	// ✅ AGGIUNGI QUESTE RIGHE - Crea directory database se non esiste
	dbDir := filepath.Dir(cfg.DB.Filename)
	if err := os.MkdirAll(dbDir, 0o755); err != nil {
		logger.WithError(err).Error("cannot create database directory")
		return fmt.Errorf("creating database directory: %w", err)
	}
	logger.Infof("database will be stored in: %s", cfg.DB.Filename)
	// ✅ FINE AGGIUNTA
